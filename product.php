...
<?php
  // Marketplace authentication is handled automatically in includes/auth.php
  // which is included in includes/header.php
  
  // Product data with detailed information (matching API structure)
  $products = [
    201 => [
      "id" => "201",
  ...
  // Handle multiple URL parameter formats: id, product_id, or slug
  $id = 0;
  if (isset($_GET['id'])) {
    $id = (int)$_GET['id'];
    ...
    // Find product by slug using actual slug field from product data
    $slug = $_GET['slug'];
  ...
  // Marketplace authentication is now handled automatically in includes/auth.php
  // via handle_marketplace_authentication() which is called in header.php
  // Get marketplace user ID for wishlist button
  $marketplaceUserId = get_marketplace_user_id();
  
  // Track visit to Group Marketplace database if user came from marketplace
  if ($marketplaceUserId) {
    // Include tracking function and execute server-side
    require_once __DIR__ . '/api/track_visit.php';
    
    // Track this visit directly to marketplace database
    trackMarketplaceVisit(
      $marketplaceUserId, 
      3, // company_id for Dreamspace Realty
      (string)$id, 
      $product['title'], 
      'Dreamspace Realty',
      session_id()
...
<div class="card">
  ...
  <p style="margin-top: 24px;">
    <?php if ($marketplaceUserId): ?>
      <button onclick="addToMarketplaceWishlist()" class="btn" style="background: #ec4899; margin-right: 8px;">
...
function addToMarketplaceWishlist() {
    ...
    const productId = <?php echo json_encode((string)$id); ?>;
    const productName = <?php echo json_encode($product['title']); ?>;
    
    // Use our proxy to avoid CORS issues (server-side request bypasses CORS)
    const proxyUrl = 'api/wishlist_proxy.php';
    
    fetch(proxyUrl + '?user_id=' + encodeURIComponent(marketplaceUserId) + '&product_id=' + encodeURIComponent(productId) + '&product_name=' + encodeURIComponent(productName))
        .then(response => {
            if (!response.ok) {
        ...
        .catch(error => {
            ...
            if (error.message.includes('Failed to fetch') || error.message.includes('ERR_NAME_NOT_RESOLVED')) {
                alert('Cannot connect to Group Marketplace. Please check if the marketplace URL is correct.');
            } else {
